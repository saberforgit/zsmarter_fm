package com.zsmarter.cordova.signaturepad.views;
...
import com.zsmarter.cordova.signaturepad.utils.Bezier;
import com.zsmarter.cordova.signaturepad.utils.ControlTimedPoints;
import com.zsmarter.cordova.signaturepad.utils.SvgBuilder;
import com.zsmarter.cordova.signaturepad.utils.TimedPoint;
import com.zsmarter.cordova.signaturepad.utils.XFResourcesIDFinder;
import com.zsmarter.cordova.signaturepad.view.ViewCompat;
import com.zsmarter.cordova.signaturepad.view.ViewTreeObserverCompat;
...
public class SignaturePad extends View {
    ...
    public  int DEFAULT_ATTR_PEN_MIN_WIDTH_PX = 1;
    public  int DEFAULT_ATTR_PEN_MAX_WIDTH_PX = 7;
    public  int layoutWidth = 700;
    ...
    public SignaturePad(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                XFResourcesIDFinder.getStyleableArray(context,"SignaturePad"),
                0, 0);

        //Configurable parameters
        try {
            mMinWidth = a.getDimensionPixelSize(XFResourcesIDFinder.getResStyleableID("penMinWidth"), convertDpToPx(DEFAULT_ATTR_PEN_MIN_WIDTH_PX));
            mMaxWidth = a.getDimensionPixelSize(XFResourcesIDFinder.getResStyleableID("penMaxWidth"), convertDpToPx(DEFAULT_ATTR_PEN_MAX_WIDTH_PX));
            mPaint.setColor(a.getColor(XFResourcesIDFinder.getResStyleableID("penColor"), DEFAULT_ATTR_PEN_COLOR));
            mVelocityFilterWeight = a.getFloat(XFResourcesIDFinder.getResStyleableID("velocityFilterWeight"), DEFAULT_ATTR_VELOCITY_FILTER_WEIGHT);
            mClearOnDoubleClick = a.getBoolean(XFResourcesIDFinder.getResStyleableID("clearOnDoubleClick"), DEFAULT_ATTR_CLEAR_ON_DOUBLE_CLICK);
        } finally {
            a.recycle();
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float eventPress = event.getPressure();
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mLastTouchX = eventX;
                mLastTouchY = eventY;
                addPoint(getNewPoint(eventX, eventY,eventPress));
    ...
    private TimedPoint getNewPoint(float x, float y,float press) {
        ...
        return timedPoint.set(x, y,press);
    }
...
//            float newWidth = strokeWidth(velocity);
            float newWidth = strokeWidth(newPoint.press);
            ...
            mPoints.add(getNewPoint(firstPoint.x, firstPoint.y,firstPoint.press));
        }
    }
    ...
    private ControlTimedPoints calculateCurveControlPoints(TimedPoint s1, TimedPoint s2, TimedPoint s3) {
        ...
        return mControlTimedPointsCached.set(getNewPoint(m1X + tx, m1Y + ty,s1.press), getNewPoint(m2X + tx, m2Y + ty,s3.press));
    }

    private float strokeWidth(float velocity) {
//        float press = velocity*velocity;
        float press = (float) Math.sqrt(velocity);
        return Math.max(mMaxWidth*press, mMinWidth);
    }
