...
public class AESHelper {
    ...
    public boolean AESCipher(
            int cipherMode, String
            sourceFilePath,
            String targetFilePath, String
                    seed) throws NoSuchProviderException
...
//此处，若不使用数组加密解密会失败，因为当byteData达不到1024个时，加密方式不同对空白字节的处理也不相同，从而导致成功与失败。
    ...
    public String encrypt(String seed, String source) {
        // Log.v(TAG, "加密前的seed=" + seed + ",内容为:" + clearText);
        byte[] result = null;
        try {
            byte[] rawkey = getRawKey(seed.getBytes());
...
//            kgen.init(128);// 使用用户提供的随机源初始化此密钥生成器，使其具有确定的密钥大小。
...
////                sr = SecureRandom.getInstance("AES",p);
//
//                // 设置一个种子,一般是用户设定的密码
////                sr.setSeed(seed);
//                // 获得一个key生成器（AES加密模式）
